...
<template>
  <header class="bg-white text-black shadow py-2">
    <div class="max-w-screen-xl mx-auto flex items-center justify-between h-12 px-4">
      ...
      <nav v-if="authStore.isAdmin" class="hidden md:flex gap-8 text-sm text-gray-700 font-medium">
        <RouterLink v-for="item in menuItems" :key="item.label" :to="item.to"
          @click="item.to === '/' ? handleHomeClick($event) : null" class="hover:text-yellow-600 transition">
          {{ item.label }}
        </RouterLink>
      </nav>

      <ProductSearchBar v-model="searchInput" @submit="search" @clear="clearSearch" />
      <div class="hidden md:flex items-center gap-4">
        ...
        <RouterLink v-if="authStore.isLoggedIn" to="/notifications"
          class="relative flex items-center justify-center mr-2" title="我的通知">
          ...
          <span v-if="notificationStore.totalUnread > 0"
            class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white z-10"></span>
        </RouterLink>
    ...
    <Drawer v-model:visible="visible" position="right">
      ...
      <div class="flex flex-col gap-5 mt-4 px-4">
        <RouterLink v-for="item in menuItems" :key="item.label" :to="item.to" @click="
          () => {
            visible = false
        ...
        <div v-if="authStore.isLoggedIn"
          ...
          通知中心
          <span v-if="notificationStore.totalUnread > 0"
            class="ml-auto bg-red-500 text-white text-xs w-2.5 h-2.5 rounded-full border-2 border-white"></span>
        </div>
        ...
        <div v-else class="mt-4 space-y-3">
          <div>
            ...
            <RouterLink to="/userprofile" @click="visible = false"
              class="flex items-center text-gray-800 hover:text-emerald-600 text-base font-medium mb-3">
              <i class="pi pi-user mr-2"></i>
              個人資料
            </RouterLink>
            <RouterLink to="/orders" @click="visible = false"
              class="flex items-center text-gray-800 hover:text-emerald-600 text-base font-medium mb-3">
              <i class="pi pi-list mr-2"></i>
              訂單中心
            </RouterLink>
            <RouterLink to="/favorites" @click="visible = false"
              class="flex items-center text-gray-800 hover:text-emerald-600 text-base font-medium mb-3">
              <i class="pi pi-heart mr-2"></i>
              收藏清單
            </RouterLink>
          </div>
          <Button label="登出" severity="danger" class="w-full" @click="logout" />
        </div>
      </div>
    </Drawer>
  </header>
