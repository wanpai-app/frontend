...
const getCurrentImage = () => {
  ...
  const allImages = [
    product.value.images.cover,
    ...(product.value.images.previews || []),
  ]
...
<template>
  ...
  <div class="max-w-screen-md mx-auto px-4 py-10 space-y-6">
      ...
      <div class="flex flex-col md:flex-row gap-10 items-start">
        <div class="w-full md:max-w-[460px] text-center mx-auto">
          <div class="relative inline-block">
            <Image v-if="getCurrentImage()" :src="getCurrentImage().imgUrl" alt="Product Image"
              imageClass="w-64 h-64 object-cover rounded mx-auto" preview />
            <div
          ...
          <div v-if="product.images" class="flex justify-center gap-2 mt-4">
            <div class="w-16 h-16 border-2 rounded cursor-pointer overflow-hidden" :class="{
              'border-blue-500': selectedImageIndex === 0,
              'border-gray-300': selectedImageIndex !== 0,
            }" @click="selectImage(0)">
              <img :src="product.images.cover.imgUrl" alt="Cover thumbnail" class="w-full h-full object-cover" />
            </div>

            <div v-for="(image, index) in product.images.previews || []" :key="image.id"
              class="w-16 h-16 border-2 rounded cursor-pointer overflow-hidden" :class="{
                'border-blue-500': selectedImageIndex === index + 1,
                'border-gray-300': selectedImageIndex !== index + 1,
              }" @click="selectImage(index + 1)">
              <img :src="image.imgUrl" :alt="`Preview thumbnail ${index + 1}`" class="w-full h-full object-cover" />
            </div>
          </div>
        </div>
        ...
        <div class="w-full md:max-w-[360px] mx-auto flex flex-col space-y-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <InputNumber v-model="quantity" :ref="inputRef" inputId="quantity" :min="1" showButtons
              buttonLayout="horizontal" decrementButtonClass="p-button-text !h-8 !w-8"
              incrementButtonClass="p-button-text !h-8 !w-8" inputClass="text-center w-12 h-8 text-sm" class="text-sm">
          ...
          <div class="flex gap-3 mt-1">
            <Button label="加入購物車" icon="pi pi-shopping-cart" class="flex-1" @click="addToCart" />
            <Button :label="product.isFavorited ? '已收藏' : '收藏'"
    ...
    <div class="bg-white shadow-md rounded-lg p-6 text-black text-base sm:text-lg leading-relaxed">
