...
<template>
  ...
  <div class="space-y-8">
    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-200 p-6">
      ...
      <div :class="[
      ...
      ]" @dragover.prevent @dragenter.prevent @drop.prevent="onDropMain" @click="$refs.coverInput.click()">
        <div v-if="
          !coverImage && !internalPendingImages.some((img) => img.isCover)
        ">
        ...
        <input type="file" accept="image/*" class="hidden" ref="coverInput" @change="onSelectMainImage" />
      </div>
      ...
      <div class="mt-6">
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="relative group w-40 h-40">
              ...
              <div
                ...
                <div class="opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                  <Button @click.stop="previewImage(coverImage.imgUrl)" severity="info" size="small" class="shadow-lg"
                    icon="pi pi-eye" rounded />
                  <Button @click.stop="deleteImage(coverImage.id)" :disabled="isUploading" severity="danger"
                    size="small" class="shadow-lg" icon="pi pi-trash" rounded />
                </div>
              </div>
            </div>
          ...
          <div v-for="(item, index) in internalPendingImages.filter(
            (img) => img.isCover
          )" :key="'pending-cover-' + index" class="relative group">
            <div class="relative w-40 h-40 mx-auto">
              <img :src="item.preview" :alt="item.file?.name || '預覽圖片'"
                ...
                @click="previewImage(item.preview)" @error="handleImageError" @load="handleImageLoad" />
              <Button @click.stop="
                removePendingImage(
                ...
                " severity="danger" size="small" class="absolute top-2 right-2 shadow-lg" icon="pi pi-trash" rounded />
            </div>
    ...
    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border border-purple-200 p-6">
      ...
      <div :class="[
      ...
      ]" @dragover.prevent @dragenter.prevent @drop.prevent="onDropPreviewImages"
        ...
        <input type="file" ref="previewInput" multiple accept="image/*" @change="onSelectPreviewImages"
          :disabled="overLimit" class="hidden" />
      </div>
      ...
      " class="mt-6">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center">
            ...
            <span class="ml-2 inline-flex items-center px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-600">
        ...
        <draggable v-model="previewImages" item-key="id"
              ...
              <div class="relative overflow-hidden rounded-xl">
                ...
                <div
                  ...
                  <div class="absolute bottom-2 left-2 right-2 flex justify-between items-end">
                    <Button @click.stop="previewImage(element.imgUrl)" severity="info" size="small" class="shadow-lg"
                      icon="pi pi-eye" rounded />
                    <Button @click.stop="deleteImage(element.id)" :disabled="isUploading" severity="danger" size="small"
                      class="shadow-lg" icon="pi pi-trash" rounded />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="internalPendingImages.filter((img) => !img.isCover).length > 0" class="mt-3">
          <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            <div v-for="(item, index) in internalPendingImages.filter(
              (img) => !img.isCover
            )" :key="'pending-preview-' + index" class="relative group">
              <div class="relative">
                <img :src="item.preview" :alt="item.file?.name || '預覽圖片'"
                ...
                <Button @click.stop="
                  ...
                  " severity="danger" size="small" class="absolute top-1 right-1 shadow-lg" icon="pi pi-trash"
                  rounded />
    ...
    <Dialog v-model:visible="showPreview" modal :style="{ width: '90vw', maxWidth: '800px' }" :maximizable="true"
      ...
      <div class="flex justify-center items-center bg-gray-50 rounded-lg p-4">
        <Image :src="previewImageUrl" alt="圖片預覽" class="max-w-full max-h-96 rounded-lg shadow-lg" preview />
      </div>
      <template #footer>
        <div class="flex justify-end">
          <Button label="關閉" icon="pi pi-times" @click="showPreview = false" severity="secondary" />
        </div>
      </template>
    </Dialog>
    <div v-if="totalImages > 0" class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border p-4">
