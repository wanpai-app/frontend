  ...
  <div class="flex justify-center items-start min-h-screen bg-gray-100 py-10">
    <div class="w-full max-w-3xl bg-white rounded-2xl shadow-xl px-10 py-8">
      ...
      <div v-if="order" class="flex flex-col gap-6 mt-2">
        ...
        <div>
          <label class="font-semibold mb-2 block text-gray-700" for="orderItems">
            訂購商品
          </label>
            ...
            <DataTable :value="orderItems" tableStyle="min-width: 100%">
              <Column field="img" header="">
                <template #body="slotProps">
                  <img :src="slotProps.data.img" alt="" class="w-14 h-14 object-cover rounded-md" />
                </template>
              </Column>
        ...
        <div>
          ...
          <div class="flex flex-col gap-4">
            <input v-model="form.recipientName" type="text" id="recipientName"
              ...
              placeholder="收件人姓名" />
            <input v-model="form.recipientPhone" type="text" id="recipientPhone"
              class="bg-gray-50 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary w-full px-4 py-2 text-lg transition"
              placeholder="收件人電話" />
            <input v-model="form.shippingAddress" type="text" id="recipientAddress"
              class="bg-gray-50 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary w-full px-4 py-2 text-lg transition"
              placeholder="收件人地址" />
          </div>
        </div>
        <div>
          ...
          <Select v-model="form.status" :options="statusOptions" optionLabel="title" optionValue="value"
            placeholder="訂單狀態" class="w-full md:w-56" />
        </div>
