    ...
    if (isCreateMode.value) {
      ...
      if (showStockEdit.value && form.value.stockEditReason) {
        await createStockLog(
          productId.value,
          {
            amountAfter: Number(form.value.stockOnHand),
            amountChange: Number(form.value.stockOnHand),
      ...
      let beforeStock = 0
      if (showStockEdit.value && form.value.stockEditReason && productId.value) {

        const oldProduct = await fetchProductById(productId.value)
        beforeStock = Number(oldProduct.stockOnHand) || 0
      }
      ...
      if (showStockEdit.value && form.value.stockEditReason && productId.value) {
        await createStockLog(
          ...
          {
            amountAfter: Number(form.value.stockOnHand),
            amountChange: Number(form.value.stockOnHand) - beforeStock,
            reason: form.value.stockEditReason,
          })
      }
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
            ...
            <div class="space-y-4">
              <div>
                ...
                <div class="relative">
                  <input v-model.number="form.stockOnHand" type="number" id="stock" min="0" :disabled="!showStockEdit"
                    :class="[
                      'w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200',
                ...
                <small v-if="formErrors.stockOnHand" class="text-red-500 mt-1 block">
                  {{ formErrors.stockOnHand }}
                </small>
              </div>
